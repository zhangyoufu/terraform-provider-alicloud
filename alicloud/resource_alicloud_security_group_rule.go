...
func resourceAliyunSecurityGroupRuleCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("source_cidr_ip"); ok {
		_v, err := compressIPv4orCIDR(v.(string))
		if err != nil {
			return err
		}
		permissionsMap["SourceCidrIp"] = _v
		id += _v
	} else if v,ok := d.GetOk("ipv6_source_cidr_ip"); ok {
		_v, err := compressIPv6orCIDR(v.(string))
		if err != nil {
			return err
		}
		permissionsMap["Ipv6SourceCidrIp"] = _v
		id += strings.Replace(_v, ":", "_", -1)
	...
	if v, ok := d.GetOk("dest_cidr_ip"); ok {
		...
		permissionsMap["DestCidrIp"] = _v
	} else if v, ok := d.GetOk("ipv6_dest_cidr_ip"); ok {
		_v, err := compressIPv6orCIDR(v.(string))
		if err != nil {
			return err
		}
		permissionsMap["Ipv6DestCidrIp"] = _v
		id += strings.Replace(_v, ":", "_", -1)
